#' The fci algorithm applied to residuals
#'
#' The fci algorithm applied to residuals of a linear mixed model(, assuming there
#' are replicates, and K = Z Z^t.)
#'
#' If `use.GBLUP = FALSE`, GBLUP residuals are used as input for the fci-stable
#' algorithm of Colombo and Maathuis (2014).
#'
#' When `use.GBLUP = TRUE`, fci-stable is applied to the GBLUP itself (i.e the
...
#'
#' @param m.max maximum size of the conditioning set, in the fci-algorithm on the
#'   residuals
#'
...
#'   class \code{"fcigen"}). If `return.pvalues = TRUE`, the output is a list
...
#'
#' @export
#' @importFrom pcalg fci gaussCItest
#' @importFrom stats cor

fciRes <- function (suffStat, K = NULL, covariates = NULL, QTLs = integer(),
                    cov.method = 'uni', use.GBLUP = FALSE, alpha = 0.01,
                    pdsep.max = Inf, return.pvalues = FALSE, verbose = FALSE)
{
  ...
  fci.res <- fci(suffStat =  list(C = cor(res), n = nrow(res)),
                 indepTest = gaussCItest, alpha = alpha,
                 labels = colnames(suffStat)[-1], verbose = verbose,
                 pdsep.max = pdsep.max, doPdsep = TRUE,
                 skel.method = "stable", type = "adaptive",
                 conservative = FALSE, maj.rule = TRUE)

  if (return.pvalues == TRUE) {
    return(list(gr = fci.res, pMax = fci.res@pMax))
  } else {
    return(fci.res)
  }
