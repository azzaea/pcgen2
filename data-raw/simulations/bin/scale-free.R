...
######################################################################
dag <- randDAG(n = p, d = N, method ="barabasi", par1=alpha, par2=NULL,
               DAG = TRUE, weighted = TRUE, 
               wFUN = list(runif, min = dagcoeff[1], max = dagcoeff[2]))
plot(dag, main = paste("Simulated Traits only network- Scale-free: E[N] =", N, "alpha =", alpha))
