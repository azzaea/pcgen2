...
simulate: datamaker_3genetic.R 
  ...
  dagcoeff:   0.1, .25, .5, .75,  1, 2   # Lower and upper limits of DAG edge weights 
  rangeGenVar: .01, .05, .1, .5, 1, 1.5, 3, 6   # Range of variances of direct genetic effects  ---------------> make it low
  rangeEnvVar: .01, .05, .1, .5, 1, 1.5, 3, 6   # Range of error variances (environment effects) ------------------> make it low
  diffVar: FALSE
  $true_dag: gsem 
  $data: nw.data 
  $K: K
...
DSC:
    ...
    run: simulate * learn 
    exec_path: bin
    output: simulation.1gtrait.identicalvar 
    replicte: 5 
