...
simulate: datamaker_3genetic.R 
  ...
  dagcoeff: 0.01, 0.1, .5, 1  # Lower and upper limits of DAG edge weights -------------> try with 1 edge, change its weight
  ######################################### Is it the strength of correlation? when it is large/small
  ######################################### Have an ideal data with strong relations between the data
  rangeGenVar: c(.01,.02), c(.1, .2), c(1,2), c(4,5)  # Range of variances of direct genetic effects  ---------------> make it low
  rangeEnvVar: c(.01,.02), c(.1, .2), c(1,2), c(4,5)  # Range of error variances (environment effects) ------------------> make it low
  $genvar: geneticCovMatrix
  $true_dag: gsem 
  $data: nw.data 
  $K: K
...
DSC:
    ...
    run: simulate * learn 
    exec_path: bin
    output: simulation_tuning_3genetic
    replicte: 1
